...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const [users] = await pool.execute(
      'SELECT id, name, email, role, phone, permissions, active, created_at FROM users WHERE id = ?',
      [req.params.id]
    );
...
router.post('/', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO users (name, email, password, role, phone, permissions, access_code, active) 
       VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
      [name, email, hashedPassword, role, phone, JSON.stringify(permissions || {}), accessCode]
    );
